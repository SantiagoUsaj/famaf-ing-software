...
@patch('models.game_models.session')
def test_discard_figure_card():
  ...
  mock_board.discard_figure_card.return_value = {"message": "Figure card discarded"}
  
  with patch("app.board", mock_board):
      response = client.put(f"/discard_figure_card/{player_id}/{game_id}/{tile_id}/{figure_card_id}")
      assert response.status_code == 200
...
@patch('models.game_models.session')
def test_discard_figure_card_card_not_in_hand(mock_session):
    # Mockear el retorno de la consulta de Game
    mock_game = MagicMock()
    mock_game.gameid = "ValidGame"
    game_id = mock_game.gameid
    mock_game.turn = "ValidPlayer,OtherPlayer"
    mock_figure_card = MagicMock()
    mock_figure_card.id = "some_figure_card_id"
    mock_session.query().filter_by().first.side_effect = [mock_game, mock_figure_card]  # Mockear que la figura no se encuentra en la mano
    ...
    response_join = client.put(f"/join_game/{player_id2}/{game_id}")
    assert response_join.json() == {"message": f"{player_name2} joined the game {game_name}"}

    response_start = client.put(f"/start_game/{player_id}/{game_id}")
    assert response_start.json() == {"message": "Game started"}

    tile_id = 1
    figure_card_id = "some_figure_card_id"  # Simulación de un ID no en la mano
...
@patch('models.game_models.session')
def test_discard_figure_card_not_your_turn(mock_session):
      # Mockear el retorno de la consulta de Game
      mock_game = MagicMock()
      mock_game.turn = "ValidPlayer"
      mock_session.query().filter_by().first.side_effect = [mock_game, MagicMock()]
      ...
      tile_id = 1
      figure_card_id = "some_figure_card_id"  # Simulación de un ID válido
    
      response = client.put(f"/discard_figure_card/{player_id2}/{game_id}/{tile_id}/{figure_card_id}")
      assert response.status_code == 409
      assert response.json() == {"detail": "It's not your turn"}

@patch('models.game_models.session')
def test_discard_figure_card_card_not_in_hand(mock_session):
  # Mockear el retorno de la consulta de Game y Players
  mock_game = MagicMock()
  mock_game.turn = "ValidPlayer"
  mock_player = MagicMock()
  mock_player.playerid = "ValidPlayer"
  mock_player2 = MagicMock()
  mock_player2.playerid = "ValidPlayer2"
  mock_session.query().filter_by().first.side_effect = [mock_game, mock_player, mock_player2, None]  # Simular que la carta no está en la mano
