...
describe("TableGames Component", () => {
    ...
    test("join button triggers join function and navigates on success", async () => {
        ...
        expect(JoinGame).toHaveBeenCalledWith(mockPlayerID, "1");
    ...
    test("displays error message on join failure", async () => {
        ...
        await waitFor(() => expect(consoleErrorSpy).toHaveBeenCalledWith("Error getting new game data", expect.any(Error)));
    ...
    test("pagination is rendered correctly", () => {
        ...
        expect(screen.getByTitle("Previous Page").closest('li')).toHaveClass('ant-pagination-disabled');
        expect(screen.getByTitle("Next Page").closest('li')).toHaveClass('ant-pagination-disabled');
    });
