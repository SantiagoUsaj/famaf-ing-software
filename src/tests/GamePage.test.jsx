...
class MockWebSocket {
    constructor(url) {
        if (!url.startsWith('ws://') && !url.startsWith('wss://') && !url.startsWith('http://') && !url.startsWith('https://')) {
            throw new SyntaxError("The URL's scheme must be either 'ws', 'wss', 'http', or 'https'.");
        }
        this.url = url;
...
describe('GamePage', () => {
    ...
    it('should handle WebSocket messages', async () => {
        ...
        global.WebSocket.prototype.dispatchEvent(new MessageEvent('message', mockMessage));
